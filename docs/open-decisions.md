...
| A6 | **구매 연동 제휴처** | ✅ 결정됨: 네이버쇼핑 검색 API로 '+1 아이템' 실제 상품(이미지/가격/구매링크) 노출 (2026-07-29). 무료·즉시 발급, 여러 쇼핑몰을 한 API로 커버. 단, 이 API는 구매 확정 콜백을 제공하지 않아 "구매 시 옷장 자동 등록"(`purchase-callback`, service-policy.md §5의 자동 등록 항목)은 여전히 별도 결정이 필요한 미해결 범위로 남는다 | Recommendation |
...
- **A6. 구매 연동 제휴처** — ✅ 결정됨: 네이버쇼핑 검색 API(`global.shopping.NaverShoppingClient`)로
  '+1 아이템' 추천명을 검색해 실제 상품(이미지/가격/구매링크)을 함께 노출 (2026-07-29). 쿠팡
  파트너스(수수료 O, 가입 심사 필요)와 무신사(공개 API 없음) 대비, 무료·즉시 발급에 여러 쇼핑몰을
  한 API로 커버할 수 있어 채택. "구매 확정 시 옷장 자동 등록"은 이 API로 불가능해 범위 밖으로 남음.